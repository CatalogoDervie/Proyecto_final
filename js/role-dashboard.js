...
function agendaPanel() {
  ...
  const tableHead = medicalAgenda
    ? '<colgroup><col class="rd-col-hour"><col class="rd-col-patient"><col class="rd-col-clinic"><col class="rd-col-eye"><col class="rd-col-lens"></colgroup><thead><tr><th>Hora</th><th>Paciente</th><th>Clínica</th><th>Ojo</th><th>Dioptría / lente</th></tr></thead>'
    : '<thead><tr><th>Hora</th><th>Paciente</th><th>DNI</th><th>Teléfono</th><th>Clínica</th><th>Ojo</th><th>Dioptría / lente</th></tr></thead>';
  const content = [...groups].slice(0,8).map(([date, items]) => { const n=countByClinic(items); return `<details class="rd-agenda-day"><summary><span>${date.split('-').reverse().join('/')}</span><strong>${items.length} cirugías</strong><small>A ${n.a} · B ${n.b}</small></summary><div class="rd-table-wrap"><table class="${medicalAgenda?'rd-medical-agenda-table':''}">${tableHead}<tbody>${items.map(r => medicalAgenda
    ? `<tr><td>${escapeHtml(r.hora||'—')}</td><td>${escapeHtml(r.nombre||'—')}</td><td>${escapeHtml(clinicLabel(r.clinica))}</td><td>${escapeHtml(r.ojo||'—')}</td><td>${escapeHtml([r.dioptria,r.model||r.lio||r.modeloLente].filter(Boolean).join(' · ')||'—')}</td></tr>`
    : `<tr><td>${escapeHtml(r.hora||'—')}</td><td>${escapeHtml(r.nombre||'—')}</td><td>${escapeHtml(r.dni||'—')}</td><td>${escapeHtml(r.tel||'—')}</td><td>${escapeHtml(clinicLabel(r.clinica))}</td><td>${escapeHtml(r.ojo||'—')}</td><td>${escapeHtml([r.dioptria,r.model||r.lio||r.modeloLente].filter(Boolean).join(' · ')||'—')}</td></tr>`).join('')}</tbody></table></div></details>`; }).join('');
  return `<section class="rd-panel rd-agenda ${medicalAgenda?'rd-agenda-medico':''}"><div class="rd-section-head"><div><h2>Agenda quirúrgica combinada</h2><p>Próximas fechas. Abrí un día para ver únicamente los datos necesarios para la atención.</p></div><strong>${rows.length} próximas</strong></div>${content || '<div class="rd-empty">No hay cirugías programadas próximas con estos filtros.</div>'}</section>`;
}
